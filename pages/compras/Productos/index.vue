...
const handleControlCierreCarrito = (done: () => void) => {
	ElMessageBox.confirm("Desea guardar el progreso?")
		.then(() => {
			done();
		})
		.catch(() => {
...
<template>
	...
	>
		<template v-slot:thead="{ th }">
			<tr style="font-size: 0.9rem" :class="levantamientoEnProceso ? `tr-registro-en-levantamiento` : `tr-registro-no-levantamiento`" class="bg-gray-50 text-gray-700 px-4 py-3 select-none grid">
				<th>CODIGO</th>
				<th>NOMBRE</th>
				<th>MARCA</th>
				<th>CATEGORIA</th>
		...
		<template v-slot:tbody="{ dataMostrada }">
			...
			>
				...
				<td v-if="levantamientoEnProceso" class="text-center" :key="stampActualizacionRegistros">
	...
	<el-dialog v-model="mostrarDetallesProductoModal" title="Detalle de Producto" width="80%">
		<div style="border-top: 1px solid gray" class="mt-0 pt-4">
			<DetallesProductoModal v-if="mostrarDetallesProductoModal" :producto-visualizado="productoMostrarDetalle" />
		</div>
	</el-dialog>
